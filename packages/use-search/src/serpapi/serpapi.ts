...
export interface BaseResponse<P = Record<string | number | symbol, never>> {
  search_metadata: {
    id: string
...
export interface GooglePagination {
  current: number
  next: string
...
export interface GoogleEngineSearchResult {
...
export type SearchParametersDevice = 'desktop' | 'tablet' | 'mobile'

export interface SearchParameters {
  engine: string
  q: string
  google_domain: string
  device?: SearchParametersDevice
}
...
export async function getJson(options: {
  ...
  // Build search params
  const defaultGoogleParams: Omit<GoogleParameters, 'q'> = {}
  const searchParams = typeof options.q === 'string'
    ? { ...defaultGoogleParams, q: options.q }
    : options.q

  // Merge all params using defu
  const mergedOptions = defu<GoogleParameters, [GoogleParameters]>(
    {
      engine: 'google',
  ...
  // Make request using ofetch
  const { timeout, ...restParams } = mergedOptions
  return await ofetch<SearchResult>('search', {
...
function queryDisplayedFromResponseOr(res: SearchResult, or: string): string {
  return res?.search_information?.query_displayed || or
}

function containsSpellingFixFromResponse(res: SearchResult): boolean {
...
function spellingFixFromResponse(res: SearchResult): string | undefined {
 ...
 *
 * Borrowed from https://github.com/transitive-bullshit/agentic/blob/main/packages/serpapi/src/serpapi-client.ts
 *
 * The reason why we're not using the `serpapi` package directly is because it depends on `https` package which is not available in any other environments other than Node.js.
 * Which for Cloudflare Workers, Vercel and Netlify, and serverless functions in general, it's not possible to use it.
 */
export async function searchGoogleByQuery(apiKey: string, q: string, options?: { location?: string, start?: number }): Promise<GoogleEngineSearchResult> {
